...
class TerminalManager :
    ...
    private val reconnectAttempts = ConcurrentHashMap<TerminalBridge, Int>()
    private val reconnectJobs = ConcurrentHashMap<TerminalBridge, Job>()

    fun resetReconnect(bridge: TerminalBridge) {
        reconnectJobs.remove(bridge)?.cancel()
        reconnectAttempts.remove(bridge)
        synchronized(pendingReconnect) { pendingReconnect.removeAll { it.get() == null || it.get() === bridge } }
    }
    ...
    fun requestReconnect(bridge: TerminalBridge) {
        reconnectJobs.remove(bridge)?.cancel()
        val attempt = reconnectAttempts.merge(bridge, 1) { old, _ -> (old + 1).coerceAtMost(6) } ?: 1
        reconnectJobs[bridge] = scope.launch {
            delay(org.connectbot.bridge.ReconnectDelay.milliseconds(attempt - 1))
            if (!bridge.isAwaitingClose()) {
                synchronized(pendingReconnect) {
                    if (pendingReconnect.none { it.get() === bridge }) pendingReconnect.add(WeakReference(bridge))
                    if (!bridge.isUsingNetwork() || connectivityMonitor.getCurrentNetworkInfo()?.isConnected == true) reconnectPending()
                }
            }
        }
    }
    ...
    private fun reconnectPending() {
        synchronized(pendingReconnect) {
            for (ref in pendingReconnect) {
                ...
                if (bridge.isAwaitingClose()) continue
                bridge.startConnection()
            }
